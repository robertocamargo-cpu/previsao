...
# Sem ajuste - data original 14/05
mask = (df['ttr_data_vencimento'] == pd.Timestamp('2026-05-14')) & (df['ttr_saldo'] > 0)
df_filtered = df[mask].copy()

print('=== TODOS OS REGISTROS DE 14/05 (SEM AJUSTE) ===')
for idx, row in df_filtered.iterrows():
    print(f"Filial: {row['fil_descricao']} | Valor: {row['ttr_valor_titulo']:.2f} | Saldo: {row['ttr_saldo']:.2f}")
